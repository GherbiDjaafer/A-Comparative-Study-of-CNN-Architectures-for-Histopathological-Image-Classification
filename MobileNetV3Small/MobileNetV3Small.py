...
class Config:
    ...
    # Experiment metadata
    EXPERIMENT_NAME = f"MobileNetV3Small_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    DROPOUT_1 = 0.3
    DROPOUT_2 = 0.2
    total_layers = len(MobileNetV3Small(include_top=False).layers)
...
def build_model():
    ...
    base_model = MobileNetV3Small(weights='imagenet',include_top=False,input_shape=(*Config.IMG_SIZE, 3))
...
def main():
    ...
    print("\n" + "="*80)
    print("MobileNetV3Small TRAINING PIPELINE")
    print("="*80 + "\n")
