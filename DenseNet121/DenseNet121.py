...
class Config:
    ...
    # Experiment metadata
    EXPERIMENT_NAME = f"DenseNet121_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    DROPOUT_1 = 0.3
    DROPOUT_2 = 0.2
    total_layers = len(DenseNet121(include_top=False).layers)
...
def build_model():
    ...
    base_model = DenseNet121(weights='imagenet',include_top=False,input_shape=(*Config.IMG_SIZE, 3))
