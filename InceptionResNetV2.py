...
class Config:
    ...
    # Experiment metadata
    EXPERIMENT_NAME = f"InceptionResNetV2_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    DROPOUT_1 = 0.3
    DROPOUT_2 = 0.2
    total_layers = len(InceptionResNetV2(include_top=False).layers)
...
def build_model():
    ...
    base_model = InceptionResNetV2(weights='imagenet',include_top=False,input_shape=(*Config.IMG_SIZE, 3))
...
def main():
    ...
    print("\n" + "="*80)
    print("InceptionV3 TRAINING PIPELINE")
    print("="*80 + "\n")
